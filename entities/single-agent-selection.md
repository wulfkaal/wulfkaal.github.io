...
`kaal:entity:single-agent-selection`

**Status.** derived

This node is assembled mechanically from the 2 claims that carry the concept tag `single-agent-selection`. It is a roster of what the corpus says under this term. It is **not** an adjudicated definition: no single statement here has been ruled canonical, and no first-appearance call has been made. Read the claims and judge for yourself.
...
- [6192998-018](https://wulfkaal.github.io/claims/6192998-018) [failure/argued] *(failure mode)* -- Selecting one agent per job by weighted random selection appears efficient but is mathematically suboptimal for quality assurance, since expected quality equals only the reputation weighted average rather than the best available output.
...
Every claim above resolves to a record carrying a verbatim source quote, the sha256 of the source PDF, and a preformatted citation. Nothing here asks to be taken on trust.
    ...
    curl -s https://wulfkaal.github.io/entities/single-agent-selection.md | sha256sum
