# kaal:claim:2922176-003

**Claim.** Platform disruptors such as Uber and Airbnb did not decentralize their industries by eliminating intermediaries; they decentralized by replacing incumbent intermediaries with algorithmic ones.

**Type.** mechanism  **Support.** argued

**Holds when.**

- transportation and hospitality industries as the worked examples

**Source quote.**

> industries respectively not by eliminating intermediaries but by replacing them.

**From.** Mark Fenwick, Wulf A. Kaal, Erik P. M. Vermeulen, *The ‘Unmediated’ and ‘Tech-Driven’ Corporate Governance of Today's Winning Companies* (2017), I. Introduction, page 4

**Cite as.** Mark Fenwick, Wulf A. Kaal, Erik P. M. Vermeulen, The ‘Unmediated’ and ‘Tech-Driven’ Corporate Governance of Today's Winning Companies (2017). SSRN: https://ssrn.com/abstract=2922176

**Verify.** sha256 of source PDF `fa4e986917d437cb1504281e7b986be2698e9507d8feaf426c9d7d1754ca882c` at https://raw.githubusercontent.com/wulfkaal/Academic-Papers/main/papers/pdf/Fenwick%20et%20al.%20-%202017%20-%20The%20%E2%80%98Unmediated%E2%80%99%20and%20%E2%80%98Tech-Driven%E2%80%99%20Corporate%20Governance%20of%20Today%27s%20Winning%20Companies.pdf

**Topics.** decentralization

**Keywords.** platform-companies, disintermediation, peer-to-peer, algorithms

**Attestations.** https://agents.wulfkaal.com/v0/attestations?claim=kaal:claim:2922176-003
